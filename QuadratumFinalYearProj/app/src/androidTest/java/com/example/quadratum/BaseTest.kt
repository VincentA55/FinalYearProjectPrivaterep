...
import com.example.quadratum.data.DataSource
...
open class BaseTest {
    ...
    val lastPosition = DataSource.exercises.size - 1

    /**
     * Check the content of a card
     *
     * @param name
     * @param sets The full sets string as it appears on the screen
     * @param description The full description string as it appears on the screen
     * @param imageResource The image resource Id
     */
    private fun hasListItemContent(name: String, sets: String, imageResource: Int) {
        onView(withText(name))
            .check(matches(isDisplayed()))
        onView(withText(sets))
            .check(matches(isDisplayed()))
    ...
    fun checkFirstPosition() {
        hasListItemContent("Dumbbell Shoulder Press", "3",
